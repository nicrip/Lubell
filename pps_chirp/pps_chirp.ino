...
const int PPSPin = 3;  // the number of the PPS Pin
...
char filename[20];
unsigned long time1,time2,et,wait;
//////////////////////////////////// SETUP
...
void setup() {
  ...
  strcpy_P(filename, PSTR("chirp7.wav"));
  wait=50000;
}
...
void playfile(char *name){
  ...
  wave.load(); // takes a file-specific amount of time
  time2 = micros();
  et = time2-time1;
  Serial.println(et); // 
  unsigned long diff = static_cast<unsigned long>(wait-et);
  Serial.println(diff);
  delay(diff/1000);
